...
export function fixJSONString(jsonString) {
    try {
        ...
        function removeTrailingCommas(str) {
            ...
            for (let i = 0; i < str.length; i++) {
                ...
                if (char === "," && !inString) {
                    ...
                    while (nextNonWhitespace < str.length &&
                        /\s/.test(str[nextNonWhitespace])) {
                        nextNonWhitespace++;
                    }
                    if (str[nextNonWhitespace] === "]" ||
        ...
        jsonString = jsonString.replace(/({|,)\s*([^"{}:,\s]+)\s*:/g, function (match, p1, p2) {
        ...
        try {
            ...
            console.error("Could not fix JSON string:", finalError, "Original string:", jsonString);
            return null;
        }
    }
}
export const isExist = (path) => new Promise((resolve, reject) => {
